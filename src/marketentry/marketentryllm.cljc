(ns marketentry.marketentryllm
  ...
  the KAZ public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-kz-entity?=" (:has-kz-entity? e)
                        " bin-verified?=" (:bin-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-kz-entity? e))
                              (:has-kz-entity? e))
                          (or (not (:requires-bin? e))
                              (:bin-verified? e)))
                   0.9 0.3)}))
