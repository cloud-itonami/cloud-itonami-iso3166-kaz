...
2. Register the client's intake: business type, target public function,
   prior filing history in Kazakhstan if any.
3. Run the advisor in read-only mode against Goszakup (goszakup.gov.kz),
   the Ministry of Finance's centralized procurement portal.
4. Compare the checklist against the client's current documentation
   (BIN, EDS certificate, eGov.kz registration).
...
- named referral relationship with Kazakhstani-licensed counsel or a
...
a working referral relationship with Kazakhstani-licensed counsel or a
registered agent for whatever licensed representation the law of
Kazakhstan requires for actual public-procurement filings.
